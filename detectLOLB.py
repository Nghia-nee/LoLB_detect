...
def main(command):
    ...
    classified_data = {}
    unclassified_data = []
    command_df = pd.DataFrame({"Command": [command]})
        ...
        for command_tokens in tokenized_commands:
            ...
            for token in command_tokens:
                if token not in model_wordVector:
                    rare_token += 1
                token_embedding = model_wordVector[token]
    ...
    if classified_data:
        for class_name, commands in classified_data.items():
            for command in commands:
                avf = avfvector_list.pop(0)
                prediction = predict_model.predict([avf])
                print("Dự đoán:", prediction[0])
    elif unclassified_data:
        print("Command nhập vào không phân lớp được.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Detect LOLB.')
    parser.add_argument('command', type=str, nargs='+', help='The command to detect.')
