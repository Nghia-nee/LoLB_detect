...
explainer = lime_tabular.LimeTabularExplainer(X_train, mode='classification', training_labels=y_train, discretize_continuous=False)

all_features = []

for i, sample in enumerate(X_train):
    explanation = explainer.explain_instance(sample, nb_model.predict_proba)
    features = explanation.as_html()
    all_features.append(features)

# Tạo file HTML
html_content = '<html><head><title>LIME Feature Explanation</title></head><body>'
for i, features in enumerate(all_features):
    html_content += f'<h2>Sample {i+1}</h2>'
    html_content += features
html_content += '</body></html>'

# Lưu nội dung vào file HTML
with open('lime_explanation_Xtrain_Lime.html', 'w') as f:
